        ...
        {certifications && certifications.length > 0 ? (
          <Row style={{ justifyContent: "center", paddingBottom: "10px" }}>
            {certifications.map((cert) => {
              // Prefer lightweight thumbnail to avoid downloading large PDF files
              const hasThumb = Boolean(cert.thumbnail_url || (!cert.is_pdf && cert.file_url));
              const docSource = hasThumb ? null : (cert.is_pdf ? cert.file_url : null);
              const imgPath = hasThumb ? (cert.thumbnail_url || cert.file_url) : null;
              const openLink = cert.credential_url || cert.file_url;
